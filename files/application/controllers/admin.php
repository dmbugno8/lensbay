...
class Admin extends CI_Controller
{
	...
	function patient($param1 = '', $param2 = '', $param3 = '')
	{
		if ($this->session->userdata('admin_login') != 1)
			redirect(base_url() . 'index.php?login');
		if ($param1 == 'create') {
			...
			redirect(base_url() . 'index.php?admin/patient', 'refresh');
		}
		if ($param1 == 'edit') {
			$data['email']   = $this->input->post('email');
			$data['phone']   = $this->input->post('phone');
	...
	function product_sub_category($param1 = '', $param2 = '')
	{
		...
		// DECLARATION: MANAGE MATERIAL SUB CATEGORY
	function material_sub_category($param1 = '', $param2 = '')
	{
		if ($this->session->userdata('admin_login') != 1)
			redirect(base_url() . 'index.php?login');
		if ($param1 == 'create') {
			$data['material_name']					= $this->input->post('material_name');			
			$data['material_manufacture_id']		= $this->input->post('material_manufacture_id');
			$data['dk_amount']						= $this->input->post('dk_amount');
			$data['material_button_size']			= $this->input->post('material_button_size');			
			$data['description'] 					= $this->input->post('description');
			$data['category_id'] 					= $this->input->post('category_id');
			$this->db->where('material_manufacture_id', $param2);			
			$this->db->insert('material', $data);
			// UPLOAD IMAGE FILE
			$product_sub_category_id = mysql_insert_id();
			move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/material_sub_category_image/' . $param2 . '.jpg');
			$this->session->set_flashdata('flash_message', 'New product material added successfully');
			redirect(base_url() . 'index.php?admin/material_sub_category', 'refresh');
		}
		if ($param1 == 'edit') {
			$data['material_name']					= $this->input->post('material_name');			
			$data['material_manufacture_id']		= $this->input->post('material_manufacture_id');
		...
		if ($param1 == 'delete') {
			if (file_exists("uploads/material_sub_category_image/" . $param2 . ".jpg")) {
				unlink("uploads/material_sub_category_image/" . $param2 . ".jpg");
			}
			$this->db->where('material_id', $param2);
			$this->db->delete('material');
			$this->session->set_flashdata('flash_message', 'Material sub-category deleted');
			redirect(base_url() . 'index.php?admin/material_sub_category', 'refresh');
		}
		$page_data['page_name']      = 'material_sub_category';
		$page_data['page_title']     = 'Material Sub-categories';
		$page_data['sub_categories'] = $this->db->get('material')->result_array();
		$this->load->view('index', $page_data);
	}
		
		// DECLARATION: MANAGE MATERIAL MANUFACTURE CATEGORY
	function material_manufacture_category($param1 = '', $param2 = '')
	{
		if ($this->session->userdata('admin_login') != 1)
			redirect(base_url() . 'index.php?login');
		if ($param1 == 'create') {
			...
			$data['description'] 					= $this->input->post('description');
			$this->db->where('material_manufacture_id', $param2);			
			$this->db->insert('material_manufacture', $data);
			
			// UPLOAD IMAGE FILE
			$product_sub_category_id = mysql_insert_id();
			move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/material_manufacture_category_image/' . $param2 . '.jpg');
			$this->session->set_flashdata('flash_message', 'New product material added successfully');
			redirect(base_url() . 'index.php?admin/material_manufacture_category', 'refresh');
		}
		if ($param1 == 'edit') {
			$data['material_manufacture_id']		= $this->input->post('material_manufacture_id');
			$data['name']							= $this->input->post('name');
			$data['description'] 					= $this->input->post('description');
			$this->db->where('material_manufacture_id', $param2);
			$this->db->update('material_manufacture', $data);
			move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/material_manufacture_category_image/' . $param2 . '.jpg');
			$this->session->set_flashdata('flash_message', 'Informations updated');
			redirect(base_url() . 'index.php?admin/material_manufacture_category', 'refresh');
		}
		if ($param1 == 'delete') {
			if (file_exists("uploads/material_sub_category_image/" . $param2 . ".jpg")) {
				unlink("uploads/material_manufacture_category_image/" . $param2 . ".jpg");
			}
			$this->db->where('material_manufacture_id', $param2);
			$this->db->delete('material_manufacture');
			$this->session->set_flashdata('flash_message', 'Material Manufacture category deleted');
			redirect(base_url() . 'index.php?admin/material_manufacture_category', 'refresh');
		}
		$page_data['page_name']      = 'material_manufacture_category';
		$page_data['page_title']     = 'Material Manufacture Sub-categories';
		$page_data['sub_categories'] = $this->db->get('material_manufacture')->result_array();
		$this->load->view('index', $page_data);
	}
	...
	function product($param1 = '', $param2 = '', $param3 = '')
	{
		...
		if ($param1 == 'create') {
			...
			$data['front_toric']   = $this->input->post('front_toric');
			$data['front_toric_price']   = $this->input->post('front_toric_price');
			$data['bi_toric']   = $this->input->post('bi_toric');
			$data['bi_toric_price']   = $this->input->post('bi_toric_price');
			$data['multifocal']   = $this->input->post('multifocal');
			$data['multifocal_price']   = $this->input->post('multifocal_price');
	...
	function customer_patient_varify($type, $customer_patient_id)
	{
		if ($type == 'customer')
			$customer_patients = $this->db->get_where('name', array(
				'customer_id' => $customer_patient_id
			))->result_array();
		if ($type == 'patient')
			$customer_patients = $this->db->get_where('last_name', array(
				'patient_id' => $customer_patient_id
			))->result_array();
		foreach ($products as $row) {
			echo '<div class="panel-footer">

	                <button class="btn btn-success btn-circle pull-right" type="button"
	                    onclick="add_product(' . $row['customer_id'] . ')"><i class="fa fa-plus"></i>
	                        </button>
	            </div>';
		}
	}
	...
	function sale_add($param1 = '')
	{
		...
		if ($param1 == 'do_add') {
			...
			$data['od_os']     = $this->input->post('od_os');
			$this->db->insert('order', $data);
	...
	// DECLARATION: GET THE SUB CATEGORY LIST OF PRODUCTS
	function get_sub_category_list_OLD($category_id)
	{
		echo '<select data-placeholder="Select a sub-category" onchange="get_product(\'sub_category\' , this.value)" 
	                class="form-control" name="sub_category_id">
	                <option value="">Select a sub-category</option>';
	...
	function get_selected_product($input_type, $input_id, $total_number)
	{
		...
		echo '<tr id="entry_row_' . $total_number . '">
			...
			<tr>
			<th>       
			      <input type="checkbox" name="od_os" id="addons-0" value="OD">OD
			</th>
			<th>
			      <input type="checkbox" name="od_os" id="addons-0" value="OS">OS
			</th>
			</tr>
	             
			<tr>
			<th>       
			      <input type="checkbox" name="addons" id="addons-0" value="Front Toric">
			      Front Toric
			</th>
			<th>       
			      <input type="checkbox" name="addons" id="addons-0" value="Bi-Toric">
			      Bi-Toric
			</th>
			<th>       
			      <input type="checkbox" name="addons" id="addons-0" value="Mulifocal">
			      Multifocal
			</th>
			<th>       
			      <input type="checkbox" name="addons" id="addons-0" value="Toric PCs">
			      Toric PCs
			</th>			
	             
	                	 	
	                	 	
	            <tr><th>Base Curve</th><td><input placeholder="Base Curve" style="width:100px;"></input></td>	
						<td><input placeholder="2nd Base curve" style="width:100px;"></input></td>	<td>	
						            			
	            <tr><th>Power</th></th><td><input placeholder="Sphere" style="width:100px;"></input></td>	
	            		<td><input placeholder="Cylinder" style="width:100px;"></input></td>	<td>	
	            				<input placeholder="Axis" style="width:100px;"></input></td></tr>				
				
	            <tr><th>Diameter</th><td><input placeholder="Diameter" style="width:100px;"></input></td></tr>	
	            
	            <tr><th>Add Power</th><td><input placeholder="Add Power" style="width:100px;"</input></td></tr>
	            
	            <tr><th>Notes</th><td><input ></input></td></tr>';
	...
	function profile_settings($param1 = '', $param2 = '', $param3 = '')
	{
		...
		$page_data['admin']      = $this->db->get_where('admin', array(
			'admin_id' => $this->session->userdata('user_id')
		))->result_array();
