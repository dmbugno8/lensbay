...
{	
	...
	function do_login()
	{
		...
		// AUTHENTICATE MANUFACTURE LOGIN
		...
		if ($query->num_rows() > 0) {
			$row = $query->row();
			$this->session->set_userdata('manufacture_login', '1');
			$this->session->set_userdata('login_type', 'manufacture');
			$this->session->set_userdata('user_id', $row->manufacture_id);
			$this->session->set_userdata('name', $row->name);
			redirect(base_url() . 'index.php?manufacture/dashboard', 'refresh');
		}
		...
		if ($query->num_rows() > 0) {
			...
			$this->session->set_userdata('user_id', $row->patient_id);
			$this->session->set_userdata('name', $row->name);
			redirect(base_url() . 'index.php?patient/dashboard', 'refresh');
		}
	...
	function reset_password()
	{
		...
		// CHECKING CREDENTIALS FOR PATIENT
		$query_patient = $this->db->get_where('patient', array(
			'email' => $email
		));
		if ($query_patient->num_rows() > 0) {
			...
			$this->session->set_flashdata('flash_message', 'New password sent to your email');
			// SENDING MAIL TO PATIENT WITH NEW PASSWORD
			$email_to     = $email;
			$new_password = $new_password;
			$this->email_model->reset_password_email($new_password, $email_to);
			redirect(base_url() . 'index.php?login');
